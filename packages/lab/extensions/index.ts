...
const ORCHESTRATE_GUIDELINES = [
  ...
  "PI payloads: ask_clarification accepts either one card {requestId,card:{questionId,question,options:[{id,label}],allowOther,required}} or one questionnaire {requestId,card:{title,description,questions:[{id,prompt,type:single_select|multiple_select,options:[{id,label}]}],submitLabel}}; submit_clarification {requestId,questionId,selectedOptionIds,freeText?}; dispatch_doctor {requestId,doctorRole,brief}; review_doctor_synthesis {requestId,workPackageId,decision}; complete_meeting {requestId,report}; cancel_meeting {requestId}.",
