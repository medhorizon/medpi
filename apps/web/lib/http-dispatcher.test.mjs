...
test("configures HTTP_PROXY, HTTPS_PROXY, and NO_PROXY for global fetch", async (t) => {
  ...
  const proxy = createServer((req, res) => {
    proxiedRequests.push({ url: req.url, host: req.headers.host });
  ...
  const proxiedRequestCount = proxiedRequests.length + connectTargets.length;
  await assert.rejects(fetch("http://bypass.invalid:9/no-proxy", {
    signal: AbortSignal.timeout(2_000),
  }));
  assert.equal(proxiedRequests.length + connectTargets.length, proxiedRequestCount);
});
